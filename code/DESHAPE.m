...
% deShape 参数设置
time_win = 128;         % 窗口长度（样本数）
overlap = time_win/2;   % 重叠长度（hop）
nfft = 256;             % FFT 点数
...
% 未滤波 deShape 分析 
S_matrix_unfilt = cell(1, num_channels);  % 存储未滤波各通道 deShape 谱
...
for ch = 1:num_channels
    ...
    [deshape, ~, ~, ~, frequency, ~] = deShape_2(signal, fs, time_win, hf, 0.1, overlap, nfft, lf, ths);
    S_matrix_unfilt{ch} = deshape;
    F_matrix_unfilt{ch} = frequency / 1e6; 
    T_matrix_unfilt{ch} = (0:size(deshape,2)-1)*overlap/fs; 
end
...
% 滤波后 deShape 分析 
S_matrix = cell(1, num_channels);  % 存储滤波后各通道 deShape 谱
...
for ch = 1:num_channels
    ...
    T_matrix{ch} = (0:size(deshape,2)-1)*overlap/fs;
end

% 对比未滤波与滤波后的 deShape 图
for ch = 1:num_channels
    figure;
    subplot(1,2,1);
...
title('NEW-deShape-MUSIC Spectrum Estimation', 'FontSize', 16, 'FontWeight', 'bold');
xlabel('Angle (degrees)', 'FontSize', 16);
ylabel('MUSIC Spectrum (dB)', 'FontSize', 16);
xticks(-90:5:90); yticks(-20:5:20);
legend('NEW-deShape-MUSIC Spectrum', 'Location', 'Best', 'FontSize', 12);
