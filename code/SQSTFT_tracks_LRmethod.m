...
% 在各通道滤波后 sqSTFT 图上叠加显示融合后的轨迹点
for ch = 1:num_channels
    figure;
    sst_plot_db = 10*log10(abs(S_matrix_filt{ch}).^2+ eps);  
    imagesc(T_matrix_filt{ch}, F_matrix_filt{ch}/1e6, sst_plot_db);
    axis xy;
    colormap jet; colorbar;
    title(['通道 ', num2str(ch), ' 融合轨迹'], 'FontSize', 14);
    xlabel('Time (s)');
    ylabel('Frequency (MHz)');
    set(gca,'FontSize',12);
    caxis([max(sst_plot_db(:))-100, max(sst_plot_db(:))]);
    hold on;
   
    for p = 1:size(time_freq_points,1)
        t_idx = time_freq_points(p,1);
        f_idx = time_freq_points(p,2);
        if t_idx <= length(T_matrix_filt{ch}) && f_idx <= length(F_matrix_filt{ch})
            t_val = T_matrix_filt{ch}(t_idx);
            f_val = F_matrix_filt{ch}(f_idx);
            plot(t_val, f_val, 'ko', 'MarkerSize', 4, 'MarkerFaceColor', 'k');
        end
    end
    hold off;
...
title('SQSTFT-tracks_LRmethod-MUSIC Spectrum Estimation', 'FontSize',16,'FontWeight','bold');
xlabel('Angle (degrees)','FontSize',16);
ylabel('MUSIC Spectrum (dB)','FontSize',16);
xticks(-90:5:90); yticks(-20:5:20); grid on;
legend('SQSTFT-tracks_LRmethod-MUSIC Spectrum','Location','Best','FontSize',12);
