...
if R_low == 0
    ...
    for cntred = 1:red
        ...
        % First level
        temp = zeros(1, NG);
        inds = mod(level{cntred}{1}.index, NG) + 1;
        vals = level{cntred}{1}.X_val;
        for k = 1:length(inds)
            temp(inds(k)) = vals(k);
        end
        inds = mod(level{cntred}{2}.index, NG) + 1;
        vals = level{cntred}{2}.X_val .* (level{cntred}{2}.lowpass);
        for k = 1:length(inds)
            temp(inds(k)) = vals(k);
        end
        ...
        for cnt = 3:nbscales{cntred}
            ...
            for cnt2 = 1:2
                temp = zeros(1, NG);
                % 采用循环赋值方式，避免重复索引导致错误
                inds1 = mod(tiling{cntred}{cnt-1}{1,cnt2}.index + pN + 1, NG) + 1;
                vals1 = tiling{cntred}{cnt-1}{1,cnt2}.X_val;
                for k = 1:length(inds1)
                    temp(inds1(k)) = vals1(k);
                end
                inds2 = mod(tiling{cntred}{cnt-1}{2,cnt2}.index + pN + 1, NG) + 1;
                vals2 = tiling{cntred}{cnt-1}{2,cnt2}.X_val;
                for k = 1:length(inds2)
                    temp(inds2(k)) = vals2(k);
                end
    ...
    for cntred = 1:red
        ...
        temp_val = wedge_end{cntred}(cut_num);
        wedge_length{cntred} = wedge_length{cntred}(cut_num+2:end);
        wedge_length{cntred} = [R_low - min(2, R_low - temp_val), min(2, R_low - temp_val), wedge_end{cntred}(cut_num+1) - R_low, wedge_length{cntred}];
        wedge_end{cntred} = wedge_end{cntred}(cut_num+1:end);
        wedge_end{cntred} = [R_low - min(2, R_low - temp_val), R_low, wedge_end{cntred}];
        ...
        level{cntred} = cell(1, nbscales{cntred});
        tiling{cntred} = cell(1, nbscales{cntred} - 1);
    end
    if nargin < 7, NG = NN; display('Sampling grid not assigned. Run with the smallest grid.'); end
    if NG < NN
    ...
    for cntred = 1:red
        ...
        for cnt = 3:nbscales{cntred}
            ...
            for cnt2 = 1:2
                temp = zeros(1, NG);
                inds = mod(tiling{cntred}{cnt-1}{1,cnt2}.index + pN + 1, NG) + 1;
                vals = tiling{cntred}{cnt-1}{1,cnt2}.X_val;
                for k = 1:length(inds)
                    temp(inds(k)) = vals(k);
                end
                inds = mod(tiling{cntred}{cnt-1}{2,cnt2}.index + pN + 1, NG) + 1;
                vals = tiling{cntred}{cnt-1}{2,cnt2}.X_val;
                for k = 1:length(inds)
                    temp(inds(k)) = vals(k);
                end
                C{cntred}{cnt-1}{cnt2} = fftshift(ifft(ifftshift(temp))) * sqrt(NG) * fac;
        ...
        temp_cell = cell(1, nbscales{cntred}-2);
        for cnt = 1:nbscales{cntred}-2
            temp_cell{cnt} = C{cntred}{cnt+1};
        end
        C{cntred} = temp_cell;
    end
end
